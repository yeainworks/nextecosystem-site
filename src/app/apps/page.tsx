...
const apps: App[] = [
  { name: "NextVPN",       description: "Быстрый VPN сервис и IT консалтинг",           category: "Инструменты",  color: "#0c4a6e", gradient: "linear-gradient(135deg, #0c4a6e 0%, #1e1b4b 100%)", url: "https://nextvpn.ru",         logo: "/logos/nextvpn.png" },
  { name: "NextLOGISTICS",description: "Выкуп и доставка товаров из-за рубежа",         category: "Повседневное", color: "#1a2e1a", gradient: "linear-gradient(135deg, #0d2626 0%, #1a2e10 50%, #2e2a08 100%)",                                      logo: "/logos/nextlogistics.png" },
  { name: "NextCRYPTO",   description: "Социальная сеть для инвесторов и крипторынки",  category: "Финансы",      color: "#0f1a0f", gradient: "linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%)", url: "https://nextforcrypto.com",  logo: "/logos/nextcrypto.png" },
  ...
  { name: "Nova",         description: "Дизайн студия для вашей жизни",                category: "Другое",       color: "#1a1020", gradient: "linear-gradient(135deg, #6b21a8 0%, #a855f7 100%)", url: "https://novaforlive.ru",     logo: "/logos/nova.png" },
  { name: "Jury",         description: "Система для онлайн курсов и обучения",         category: "Другое",       color: "#0a1a2e", gradient: "linear-gradient(135deg, #0a1a2e 0%, #0d2a4a 100%)", url: "https://juryapp.org",        logo: "/logos/jury.png" },
];

const cats: Cat[] = ["Все", ...sections.map((s) => s.id)];
...
function AppRow({ app, first }: { app: App; first: boolean }) {
  return (
    <a
      href={app.url ?? "#"}
    ...
    >
      ...
      <div>
        ...
        <div style={{ fontSize: 13, color: "#f5f5f7" }}>{app.description}</div>
      </div>
...
export default function AppsPage() {
  ...
  const visibleSections = active === "Все" ? sections : sections.filter((s) => s.id === active);
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 clamp(16px,5vw,40px)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 52 }}>
          {visibleSections.map((sec, si) => {
            const secApps = apps.filter((a) => a.category === sec.id);
            ...
            return (
              <FadeIn key={sec.id} delay={si * 0.06}>
                ...
                <div>{secApps.map((app, i) => <AppRow key={app.name} app={app} first={i === 0} />)}</div>
              </FadeIn>
            );
          })}
        </div>

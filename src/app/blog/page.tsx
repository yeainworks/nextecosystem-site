...
function FeaturedCard({ post, large, readLabel, lang }: { post: Post; large?: boolean; readLabel: string; lang: string }) {
  if (large) {
    return (
      <div className="feat-large-card" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0, background: "#111", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, overflow: "hidden" }}>
        ...
        <div className="feat-large-text" style={{ padding: "36px 36px 36px 40px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 12, color: "rgba(255,255,255,0.4)" }}>{formatDate(post.date, lang)}</span>
          </div>
    ...
    <div className="feat-small" style={{ background: "#111", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 16, overflow: "hidden", cursor: "pointer" }}>
      ...
      <div style={{ padding: "20px 20px 24px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
          ...
          <span style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>{formatDate(post.date, lang)}</span>
        </div>
...
function PostRow({ post, first, lang }: { post: Post; first: boolean; lang: string }) {
  return (
...
export default function BlogPage() {
  ...
  const [cat, setCat] = useState<string>(t.blog.cats.all);
  const [visible, setVisible] = useState(8);
  const [loading, setLoading] = useState(true);

  const CATS = [
    { key: t.blog.cats.all,         value: t.blog.cats.all },
    { key: "Анонс",                  value: t.blog.cats.announce },
    { key: "Продукт",                value: t.blog.cats.product },
    { key: "Партнёрство",            value: t.blog.cats.partnership },
    { key: "Компания",               value: t.blog.cats.company },
  ];
  ...
  const isAllCat = cat === t.blog.cats.all || CATS.find(c => c.value === cat)?.key === t.blog.cats.all;
  const activeCatEntry = CATS.find(c => c.value === cat);
  const filtered = isAllCat
    ? posts
    : posts.filter(p => activeCatEntry && p.category === activeCatEntry.key);
    ...
    <div style={{ background: "#0a0a0a", color: "#f5f5f7", minHeight: "100vh", paddingTop: 80 }}>
      <style>{`
        ...
        @media (max-width: 768px) {
          .feat-large-card { grid-template-columns: 1fr !important; }
          .feat-large-card > div:first-child { min-height: 200px !important; }
          .feat-large-text { padding: 24px !important; }
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 clamp(16px,5vw,40px)" }}>
        ...
        <FadeIn>
          <div style={{ padding: "52px 0 40px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            <h1 style={{ fontSize: "clamp(36px, 5vw, 56px)", fontWeight: 700, letterSpacing: "-0.04em", color: "#f5f5f7", marginBottom: 10 }}>{t.blog.h1}</h1>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.5)" }}>{t.blog.subtitle}</p>
          </div>
        </FadeIn>
        ...
        {!loading && featured.length > 0 && (
          <div style={{ padding: "40px 0" }}>
            <FadeIn>
              <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.4)", marginBottom: 20 }}>{t.blog.featured}</p>
            </FadeIn>
            <FadeIn delay={0.06}>
              <div style={{ marginBottom: 12 }}>
                <FeaturedCard post={featured[0]} large readLabel={t.blog.read} lang={lang}/>
              </div>
            </FadeIn>
            {featured.length > 1 && (
              <FadeIn delay={0.1}>
                <div className="feat-small-grid" style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(featured.length - 1, 3)}, 1fr)`, gap: 12, marginTop: 12 }}>
                  {featured.slice(1, 4).map(p => <FeaturedCard key={p.id} post={p} readLabel={t.blog.read} lang={lang}/>)}
                </div>
              </FadeIn>
            )}
          </div>
        ...
        <FadeIn>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, gap: 16, flexWrap: "wrap" }}>
            <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.4)" }}>{t.blog.all}</p>
            <div style={{ display: "flex", gap: 4, overflowX: "auto", scrollbarWidth: "none" }}>
              {CATS.map(c => (
                <button key={c.key} className={`cat-tab${cat === c.value ? " active" : ""}`} onClick={() => { setCat(c.value); setVisible(8); }}>{c.value}</button>
              ))}
            </div>
          </div>
        </FadeIn>
        ...
        {loading ? (
          <div style={{ padding: "60px 0", textAlign: "center", color: "rgba(255,255,255,0.3)", fontSize: 14 }}>{t.blog.loading}</div>
        ) : shown.length === 0 ? (
          <div style={{ padding: "60px 0", textAlign: "center", color: "rgba(255,255,255,0.3)", fontSize: 14 }}>{t.blog.empty}</div>
        ) : (
          <>
            {shown.map((p, i) => (
              <FadeIn key={p.id} delay={i * 0.04}>
                <PostRow post={p} first={i === 0} lang={lang}/>
              </FadeIn>
            ))}
